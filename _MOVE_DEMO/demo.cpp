...
AC_IMPLEMENT_EXTENSION_MODULE(DemoApp)
...
#ifdef NEVER
AFX_EXTENSION_MODULE MainRenderExtDLL = { NULL, NULL };
#endif
...
BOOL WINAPI DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved) {
	...
	if (dwReason == DLL_PROCESS_ATTACH) {
		...
		//DemoApp.AttachInstance(hInstance);
		InitAcUiDLL();
	}
	else if (dwReason == DLL_PROCESS_DETACH) {
		DemoApp.DetachInstance();
	}
	return (TRUE);
}
